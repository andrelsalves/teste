...
const TechDashboard: React.FC<TechDashboardProps> = ({
  ...
  // Carregamento dinâmico seguro para evitar Erro #130 em produção
  useEffect(() => {
    const loadSignatureLib = async () => {
      try {
        const mod = await import('react-signature-canvas');
        setSigComponent(() => mod.default || mod);
      } catch (err) {
        console.error("Erro ao carregar SignatureCanvas:", err);
      }
    };
    loadSignatureLib();
  }, []);
