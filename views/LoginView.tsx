...
export default function LoginView() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // O redirecionamento acontece automaticamente via AuthProvider -> App.tsx
    } catch (err: any) {
      console.error(err);
      setErrorMsg(err.message || 'Credenciais inválidas ou erro de conexão.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-[#0f172a] flex flex-col items-center justify-start pt-20 p-6">
      ...
      <div className="w-full max-w-[400px] bg-[#1e293b]/40 backdrop-blur-md border border-white/5 rounded-[32px] p-10 shadow-2xl animate-slideUp">
        ...
        {/* Seletor Visual de Role */}
        <div className="flex p-1 bg-[#0b1120] rounded-[18px] mb-10 border border-slate-800">
          <button 
            type="button"
            onClick={() => setActiveRole(UserRole.EMPRESA)} 
            className={`flex-1 py-3 text-[10px] font-black rounded-[14px] transition-all ${
              activeRole === UserRole.EMPRESA ? 'bg-emerald-500 text-slate-900 shadow-lg' : 'text-slate-500'
            }`}
          >
            EMPRESA
          </button>
          <button 
            ...
            className={`flex-1 py-3 text-[10px] font-black rounded-[14px] transition-all ${
              activeRole === UserRole.TECNICO ? 'bg-emerald-500 text-slate-900 shadow-lg' : 'text-slate-500'
            }`}
          >
            TÉCNICO
          </button>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:bg-slate-800 disabled:text-slate-600 text-slate-900 font-black py-5 rounded-[20px] shadow-lg shadow-emerald-500/20 uppercase text-[11px] tracking-widest transition-all active:scale-[0.98] mt-4"
          >
            {loading ? (
